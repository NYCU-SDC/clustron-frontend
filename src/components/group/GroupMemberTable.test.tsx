...
describe("GroupMemberTable", () => {
  ...
  describe("Member Actions", () => {
    it("should call onRemove when member is deleted", async () => {
      ...
      const moreButtons = screen.getAllByRole("button", { name: "" }); // MoreHorizontal icon has no accessible name
      const moreButton = moreButtons.find(
        (button) => button.querySelector("svg") && !button.textContent, // Make sure it's just an icon button
      );
